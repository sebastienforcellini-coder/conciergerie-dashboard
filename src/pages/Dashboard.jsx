...
const IC = {
  ...
  people:<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>,
};
...
function PeopleBadge({ guests, dark }) {
  const n = Number(guests) || 0;
  return (
    <div style={{ display:"flex", alignItems:"center", gap:4, background:dark?"#1a1a2e":"#f5f5f5", color:dark?"white":"#6b7280", fontSize:11, fontWeight:600, padding:"3px 9px", borderRadius:99, border:dark?"none":"1px solid #e5e7eb" }}>
      {IC.people}
      {n > 0 ? `${n} pers.` : "? pers."}
    </div>
  );
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14,marginBottom:14}}>
        ...
        <div style={card}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
            ...
            <div style={{display:"flex",gap:6}}>
              <span style={{background:"#E1F5EE",color:"#085041",fontSize:11,fontWeight:500,padding:"3px 10px",borderRadius:99}}>{checkinsToday.length} arrivée{checkinsToday.length!==1?"s":""}</span>
              <span style={{background:"#F1EFE8",color:"#5F5E5A",fontSize:11,fontWeight:500,padding:"3px 10px",borderRadius:99}}>{checkoutsToday.length} départ{checkoutsToday.length!==1?"s":""}</span>
            </div>
          </div>

          {checkinsToday.length===0&&checkoutsToday.length===0 && (
            <div style={{fontSize:13,color:"#c0c0c0",padding:"20px 0",textAlign:"center"}}>Aucun mouvement aujourd'hui</div>
          )}

          {checkinsToday.length>0 && (
            <div style={{marginBottom:checkoutsToday.length>0?16:0}}>
              <div style={{fontSize:11,color:"#0F6E56",fontWeight:600,marginBottom:8,display:"flex",alignItems:"center",gap:5}}>
              ...
              {checkinsToday.map(b => {
                ...
                return (
                  <div key={b.id} style={brow}>
                    <div style={{width:34,height:34,borderRadius:"50%",background:plt.bg,color:plt.color,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,flexShrink:0}}>{(b.name||"?")[0].toUpperCase()}</div>
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:6,marginTop:5}}>
                        <PeopleBadge guests={b.guests} dark={true}/>
          ...
          {checkoutsToday.length>0 && (
            <div style={{paddingTop:checkinsToday.length>0?14:0,borderTop:checkinsToday.length>0?"1px solid #f5f5f5":"none"}}>
              <div style={{fontSize:11,color:"#5F5E5A",fontWeight:600,marginBottom:8,display:"flex",alignItems:"center",gap:5}}>
              ...
              {checkoutsToday.map(b => {
                ...
                return (
                  <div key={b.id} style={brow}>
                    <div style={{width:34,height:34,borderRadius:"50%",background:"#f5f5f5",color:"#888",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,flexShrink:0}}>{(b.name||"?")[0].toUpperCase()}</div>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:13,fontWeight:600,color:"#1a1a2e"}}>{b.name||"—"}</div>
                      <div style={{fontSize:11,color:"#9ca3af",marginTop:2}}>{prop?.name||"—"} · {b.nights||"?"} nuit{(b.nights||0)>1?"s":""} passée{(b.nights||0)>1?"s":""}</div>
                      <div style={{display:"flex",alignItems:"center",gap:6,marginTop:5}}>
                        <PeopleBadge guests={b.guests} dark={false}/>
                        <span style={{fontSize:10,background:plt.bg,color:plt.color,padding:"2px 8px",borderRadius:99,fontWeight:500}}>{b.platform}</span>
                      </div>
                    </div>
                    <div style={{textAlign:"right",flexShrink:0}}>
        ...
        <div style={card}>
          ...
          <div style={{display:"flex",gap:3,marginBottom:8}}>
            {week.map(d => (
              <div key={d} style={{flex:1,textAlign:"center",fontSize:10,color:d===today?"#1a1a2e":"#c0c0c0",fontWeight:d===today?700:400}}>
                {DAYS[new Date(d).getDay()]}<br/>{new Date(d).getDate()}
              </div>
            ))}
          </div>
